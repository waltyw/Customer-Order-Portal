...
SET @q = (SELECT IF(COUNT(*) = 0,
  'ALTER TABLE `users` ADD COLUMN `cc_email_1` VARCHAR(255) DEFAULT NULL AFTER `email`',
  'SELECT 1') FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'cc_email_1');
PREPARE s FROM @q; EXECUTE s; DEALLOCATE PREPARE s;

SET @q = (SELECT IF(COUNT(*) = 0,
  'ALTER TABLE `users` ADD COLUMN `cc_email_2` VARCHAR(255) DEFAULT NULL AFTER `cc_email_1`',
  'SELECT 1') FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'cc_email_2');
PREPARE s FROM @q; EXECUTE s; DEALLOCATE PREPARE s;
